#include "ros/ros.h"
#include "service_tutorial/AddTwoInts.h" // 서비스 헤더 파일, 빌드 시 생성
...
int main(int argc, char** argv)
{
    ...
    // 서비스 이름은 (add_two int)이고 패키지 (service_tutorial)의 (AddTwoInts) 서비스 파일을 사용한다.
    ...
    if (my_client.call(srv)) // 통신이 잘 된건지 확인
    {
        ROS_INFO("send srv: srv.request.a=%d, srv.request.b=%d", srv.request.a, srv.request.b);
        ROS_INFO("receive srv: srv.responce.result=%d", srv.response.result);
    }
